...
All three parameters are REQUIRED. Calls missing acceptance_criteria or
stop_conditions will be REJECTED.
...
is inherently biased. Instead, use the IMPLEMENT-THEN-VERIFY pattern:
  ...
  Step 3: execute(
    ...
    stop_conditions="two equivalent source/tool attempts produce no new
      entity IDs or require manual login/CAPTCHA"
  )
...
- Do not keep a claim open after two equivalent tool batches add no
  claim-relevant delta. Synthesize the current decision instead.
