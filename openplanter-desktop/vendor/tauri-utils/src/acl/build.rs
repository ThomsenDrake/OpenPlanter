...
pub fn define_permissions<F: Fn(&Path) -> bool>(
    ...
    let permission_files = glob::glob(pattern)?
        ...
        .filter(|p| !is_appledouble_sidecar(p))
    ...
    for (key, value) in env::vars_os() {
            ...
            })
        {
            ...
            let permissions_str = fs::read_to_string(&permissions_path)
                .map_err(|e| Error::ReadFile(e, permissions_path))?;
...
pub fn generate_docs(
    ...
    for permission in permissions {
        ...
        if let Some(default) = &permission.default {
            ...
            if !default.permissions.is_empty() {
                default_permission
                    .push_str("#### This default permission set includes the following:\n\n");
    ...
    let docs =
        format!("{default_permission}{PERMISSION_TABLE_HEADER}\n{permission_table}</table>\n");
...
mod tests {
    ...
    #[test]
    fn define_permissions_ignores_appledouble_sidecars() {
        let root = temp_path("permissions");
        let permissions_dir = root.join("permissions");
        let out_dir = root.join("out");
        fs::create_dir_all(&permissions_dir).expect("should create permissions dir");
        fs::create_dir_all(&out_dir).expect("should create out dir");

        fs::write(
            permissions_dir.join("camera.toml"),
            r#"[[permission]]
identifier = "allow-camera"
description = "Enables the camera command."
commands.allow = ["camera"]
"#,
        )
        .expect("should write permission file");
        fs::write(permissions_dir.join("._camera.toml"), [0xFF, 0xD8, 0xFF])
            .expect("should write appledouble sidecar");

        let permissions = define_permissions(
            &format!(
                "{}/**/*.toml",
                glob::Pattern::escape(&permissions_dir.to_string_lossy())
            ),
            "tauri:test",
            &out_dir,
            |_| true,
        )
        .expect("should ignore appledouble sidecars");
