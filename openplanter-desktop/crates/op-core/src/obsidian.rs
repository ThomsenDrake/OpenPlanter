...
pub const DEFAULT_OBSIDIAN_EXPORT_SUBDIR: &str = "Cestus";
const COPIED_ARTIFACT_MAX_BYTES: u64 = 2_000_000;
const COPIED_TEXT_EXTENSIONS: &[&str] = &["md", "txt", "json", "csv", "tsv", "yaml", "yml"];
...
struct CopiedContent {
    ...
    wiki_index: Option<String>,
    artifact_index: Option<String>,
}
...
pub fn markdown_file_link(label: &str, target: &str) -> String {
    format!(
        "[{}]({})",
        markdown_label(label),
        percent_encode_markdown_file_target(target)
    )
}
...
pub fn export_investigation_pack(
    ...
    let session_result = read_latest_session_result(session_dir)?;
    let mut copied_content =
        collect_copied_content(workspace, &target_root, generated_at.as_str())?;
    ...
    let mut files: Vec<(PathBuf, String)> = vec![
        ...
        (
            investigation_dir.join("Session Result.md"),
            render_session_result(
                session_result.as_deref(),
                &investigation_id,
                &session_id,
                &generated_at,
            ),
        ...
        (
            investigation_dir.join("Content Index.md"),
    ...
    files.append(&mut copied_content.files);
...
fn render_home(
    ...
    let mut lines = vec![
        ...
        format!("# Investigation Home: {investigation_id}"),
        String::new(),
        "> Generated by Cestus from `investigation_state.json`. Edits here are not merged back into Cestus.".to_string(),
        String::new(),
        "## Navigation".to_string(),
        format!("- {}", markdown_link("Session Result", "Session Result.md")),
        ...
        format!("- {}", markdown_link("Content Index", "Content Index.md")),
    ...
    if let Some(wiki_index) = &copied_content.wiki_index {
        lines.push(format!(
            "- {}",
            markdown_file_link("Copied Cestus Wiki", &format!("../../{wiki_index}"))
        ));
    }
    if let Some(artifact_index) = &copied_content.artifact_index {
        lines.push(format!(
            "- {}",
            markdown_file_link(
    ...
    lines.extend([
        ...
        format!(
            "- Full final output: {}",
            markdown_link("Session Result", "Session Result.md")
        ),
        format!(
            "- Copied source material: {}",
            markdown_link("Content Index", "Content Index.md")
        ),
    ]);
...
fn render_session_result(
    ...
    let mut lines = vec![
        ...
        "# Session Result".to_string(),
        String::new(),
        format!("Back to {}", markdown_link("Home", "Home.md")),
        String::new(),
        "The final Cestus response is embedded here so the generated pack remains useful inside Obsidian without hidden runtime files.".to_string(),
        String::new(),
    ];
    match session_result
        ...
        .filter(|value| !value.is_empty())
    {
        Some(result) => lines.push(result.to_string()),
        None => lines.push("_No final session result was found in the session logs._".to_string()),
    }
    finish(lines)
}
...
fn render_evidence(
    ...
    let mut lines = vec![
        ...
        String::new(),
        format!(
            "For the synthesized final findings, see {}.",
            markdown_link("Session Result", "Session Result.md")
        ),
        String::new(),
    ];
    let evidence_records = state
        .evidence
        .iter()
        .filter(|(_, record)| should_export_evidence_record(record))
        .collect::<Vec<_>>();
    if evidence_records.is_empty() {
        lines
            .push("- _No non-trace evidence records were present in the typed state._".to_string());
    }
    for (evidence_id, record) in evidence_records {
        ...
        if let Some(target) = first_target(record) {
            ...
            lines.push(format!(
                "- **Source**: {}",
                source_reference(label, &target, copied_content)
            ));
        }
...
fn render_content_index(
    ...
    let keywords = content_keywords(investigation_id, &state.objective);
    let relevant_wiki = relevant_copied_paths(&copied_content.wiki_files, &keywords);
    let relevant_artifacts = relevant_copied_paths(&copied_content.artifact_files, &keywords);
    let mut lines = vec![
        frontmatter(
            "content_index",
            investigation_id,
        ...
        format!("Back to {}", markdown_link("Home", "Home.md")),
        String::new(),
        "Copied content that now lives inside this Obsidian export folder. Hidden runtime state, replay logs, settings, and credentials are not copied.".to_string(),
        String::new(),
        "## Top-Level Copied Areas".to_string(),
    ];
    if let Some(wiki_index) = &copied_content.wiki_index {
        lines.push(format!(
            "- {}",
    ...
    if copied_content.wiki_index.is_none() && copied_content.artifact_index.is_none() {
        lines.push("- _No copied content areas were found._".to_string());
    }

    lines.extend([String::new(), "## Relevant Wiki Notes".to_string()]);
    push_relevant_links(&mut lines, &relevant_wiki, "Wiki", 80);

    lines.extend([String::new(), "## Relevant Workspace Artifacts".to_string()]);
    push_relevant_links(&mut lines, &relevant_artifacts, "Workspace-Artifacts", 120);
...
fn render_sessions(
    ...
    let mut lines = vec![
        ...
        format!("Back to {}", markdown_link("Home", "Home.md")),
        String::new(),
        "Cestus runtime logs are not copied into the Obsidian vault. Use the session result and generated Markdown notes for the readable investigation surface.".to_string(),
        String::new(),
        format!("- {}", markdown_link("Session Result", "Session Result.md")),
        String::new(),
...
fn render_index(
    ...
    let mut lines = vec![
        frontmatter(
            ...
            "workspace",
            generated_at,
            "Cestus Investigations",
        ),
        "# Cestus Investigations".to_string(),
        String::new(),
        "> Generated by Cestus. This folder is safe to place inside an Obsidian vault.".to_string(),
        String::new(),
    ...
    if copied_content.wiki_index.is_some() || copied_content.artifact_index.is_some() {
        lines.extend([
            String::new(),
            "## Included Content".to_string(),
            String::new(),
        ]);
        if let Some(wiki_index) = &copied_content.wiki_index {
            lines.push(format!(
                "- {}",
                markdown_file_link("Copied Cestus Wiki", wiki_index)
            ));
        }
        if let Some(artifact_index) = &copied_content.artifact_index {
            lines.push(format!(
                "- {}",
                markdown_file_link("Copied Workspace Artifacts", artifact_index)
            ));
        }
    }
    Ok(finish(lines))
...
fn render_canvas(
    ...
    let file_nodes = [
        ...
        ("file-session-result", "Session Result.md", 420, 0),
        ("file-findings", "Findings.md", 840, 0),
        ("file-questions", "Questions.md", 1260, 0),
        ("file-evidence", "Evidence.md", 0, 340),
        ("file-content-index", "Content Index.md", 420, 340),
        ("file-tasks", "Tasks.md", 840, 340),
        ("file-sessions", "Sessions.md", 1260, 340),
    ];
...
fn collect_copied_content(
    workspace: &Path,
    target_root: &Path,
    generated_at: &str,
) -> Result<CopiedContent, ObsidianExportError> {
    let mut copied = CopiedContent::default();
    collect_wiki_files(workspace, target_root, &mut copied)?;
    collect_workspace_artifacts(workspace, target_root, &mut copied)?;
    if !copied.artifact_files.is_empty() {
        let index_rel = "Workspace-Artifacts/Index.md".to_string();
        let index_path = target_root.join(&index_rel);
        let index = render_artifact_index(&copied.artifact_files, generated_at);
        copied.files.push((index_path, index));
        copied.artifact_index = Some(index_rel);
    }
    copied
        .files
...
fn collect_wiki_files(
    workspace: &Path,
    target_root: &Path,
    copied: &mut CopiedContent,
) -> Result<(), ObsidianExportError> {
    let wiki_root = workspace.join(".openplanter").join("wiki");
    if !wiki_root.exists() {
        return Ok(());
    }
    for entry in WalkDir::new(&wiki_root).into_iter().filter_map(Result::ok) {
        ...
        let dest_rel = format!("Wiki/{rel}");
        let content = read_text_lossy(entry.path())?;
        copied.files.push((target_root.join(&dest_rel), content));
        copied.wiki_files.push(dest_rel.clone());
        if dest_rel == "Wiki/index.md" {
            copied.wiki_index = Some(dest_rel);
        }
    }
    copied.wiki_files.sort();
    Ok(())
}
...
fn collect_workspace_artifacts(
    ...
    let walker = WalkDir::new(workspace)
        .into_iter()
        .filter_entry(|entry| should_descend_workspace_artifacts(entry, workspace, target_root));
    for entry in walker.filter_map(Result::ok) {
        if !entry.file_type().is_file() || !should_copy_workspace_artifact(entry.path()) {
            continue;
        }
        ...
        let rel = path_to_slash(rel);
        let dest_rel = format!("Workspace-Artifacts/{rel}");
        let content = read_text_lossy(entry.path())?;
        copied.files.push((target_root.join(&dest_rel), content));
        copied.artifact_files.push(dest_rel);
    }
    copied.artifact_files.sort();
    Ok(())
}

fn should_descend_workspace_artifacts(
    entry: &DirEntry,
    workspace: &Path,
    target_root: &Path,
) -> bool {
    let path = entry.path();
    if path != workspace && path.starts_with(target_root) {
        return false;
    }
    if path == workspace || !entry.file_type().is_dir() {
        return true;
    }
...
fn should_copy_workspace_artifact(path: &Path) -> bool {
    let visible_file = path
        .file_name()
    ...
    visible_file
        ...
        && path
            .metadata()
            .map(|metadata| metadata.len() <= COPIED_ARTIFACT_MAX_BYTES)
            .unwrap_or(false)
}
...
fn read_text_lossy(path: &Path) -> Result<String, ObsidianExportError> {
    let bytes = fs::read(path).map_err(|source| ObsidianExportError::ReadFile {
        path: path.display().to_string(),
        source,
    })?;
...
fn render_artifact_index(artifact_files: &[String], generated_at: &str) -> String {
    let mut groups: BTreeMap<String, Vec<String>> = BTreeMap::new();
    for rel in artifact_files {
        let artifact_rel = rel
            .strip_prefix("Workspace-Artifacts/")
            .unwrap_or(rel.as_str());
        let group = artifact_rel
            .split('/')
            .next()
            .filter(|_| artifact_rel.contains('/'))
            .unwrap_or("Root");
        groups
            .entry(group.to_string())
            .or_default()
            .push(artifact_rel.to_string());
    }
    let mut lines = vec![
        frontmatter(
            "artifact_index",
            ...
            "workspace",
            generated_at,
            "Cestus Workspace Artifacts",
        ),
        "# Cestus Workspace Artifacts".to_string(),
        String::new(),
        "Text artifacts copied from the Cestus workspace. Hidden runtime files, credentials, replay logs, node modules, and oversized raw files are excluded.".to_string(),
        String::new(),
    ];
    for (group, files) in groups {
        ...
        lines.push(String::new());
        for file in files {
            lines.push(format!("- {}", markdown_file_link(&file, &file)));
        }
        lines.push(String::new());
    }
    finish(lines)
...
fn read_latest_session_result(session_dir: &Path) -> Result<Option<String>, ObsidianExportError> {
    let candidates = [
        (session_dir.join("events.jsonl"), "turn.completed"),
        (session_dir.join("replay.jsonl"), "assistant.message"),
    ];
    for (path, event_type) in candidates {
        let Some(result) = read_latest_text_event(&path, event_type)? else {
            continue;
        };
...
fn read_latest_text_event(
    path: &Path,
    event_type: &str,
) -> Result<Option<String>, ObsidianExportError> {
    if !path.exists() {
        return Ok(None);
    }
    let content = fs::read_to_string(path).map_err(|source| ObsidianExportError::ReadFile {
    ...
    for line in content.lines() {
        ...
        if value_str(&value, "event_type") != Some(event_type) {
            continue;
        }
        let payload = value.get("payload").unwrap_or(&value);
        let Some(text) = value_str(payload, "text") else {
            continue;
        };
...
fn content_keywords(investigation_id: &str, objective: &str) -> Vec<String> {
    let mut keywords = BTreeSet::new();
    for token in [investigation_id, objective]
        .join(" ")
...
fn relevant_copied_paths(paths: &[String], keywords: &[String]) -> Vec<String> {
    if keywords.is_empty() {
        return paths.iter().take(40).cloned().collect();
    }
    paths
        .iter()
        .filter(|path| {
...
fn push_relevant_links(lines: &mut Vec<String>, paths: &[String], prefix: &str, limit: usize) {
    if paths.is_empty() {
        lines.push("- _No matching copied files found._".to_string());
        return;
    }
    for rel in paths.iter().take(limit) {
        let label = rel
            .strip_prefix(&format!("{prefix}/"))
            .unwrap_or(rel.as_str());
        lines.push(format!(
            "- {}",
            markdown_file_link(label, &format!("../../{rel}"))
        ));
    }
    if paths.len() > limit {
        lines.push(format!(
            "- _{} additional matching files omitted from this index; browse the copied folder._",
            paths.len() - limit
        ));
    }
}
...
fn should_export_evidence_record(record: &Value) -> bool {
    let body = ["content", "text", "description"]
        .iter()
        .find_map(|key| value_str(record, key))
        .unwrap_or("");
    let body = body.trim();
    if body.starts_with("[trace]") || body.starts_with("[curator]") || body.contains("state.json#")
    {
        return false;
    }
    true
...
fn source_reference(label: &str, target: &str, copied_content: &CopiedContent) -> String {
    if is_external_markdown_target(target) {
        markdown_link(label, target)
    } else if let Some(copied_target) = copied_content_link_target(target, copied_content) {
        markdown_file_link(label, &copied_target)
    } else {
        format!("`{}`", inline_code(target))
    }
}

fn copied_content_link_target(target: &str, copied_content: &CopiedContent) -> Option<String> {
    let target = target.trim().replace('\\', "/");
    if target.is_empty() || target.starts_with('/') {
        return None;
    }
    let target = target.trim_start_matches("./");
    if let Some(wiki_rel) = target.strip_prefix(".openplanter/wiki/") {
        let copied_rel = format!("Wiki/{wiki_rel}");
        if copied_content.wiki_files.binary_search(&copied_rel).is_ok() {
            return Some(format!("../../{copied_rel}"));
        }
    }
    let copied_rel = format!("Workspace-Artifacts/{target}");
    if copied_content
        .artifact_files
        .binary_search(&copied_rel)
        .is_ok()
    {
        return Some(format!("../../{copied_rel}"));
    }
...
fn percent_encode_markdown_file_target(target: &str) -> String {
    let mut out = String::new();
    for byte in target.as_bytes() {
...
mod tests {
    ...
    fn subdir_normalization_defaults_empty_results() {
        ...
        assert_eq!(
            normalize_obsidian_export_subdir(Some("/Research/Cestus/")),
            "Research/Cestus"
        );
    }
    ...
    fn target_root_supports_fresh_and_existing_vault_modes() {
        ...
        config.subdir = "Research/Cestus".to_string();
        assert_eq!(
            target_root(workspace, &config).unwrap(),
            PathBuf::from("/tmp/Vault/Research/Cestus")
        );
    }
    ...
    fn export_pack_writes_markdown_manifest_and_canvas() {
        ...
        let wiki_dir = workspace.path().join(".openplanter/wiki");
        fs::create_dir_all(&wiki_dir).unwrap();
        fs::write(
            wiki_dir.join("index.md"),
            "# Wiki Index\n\nDurable investigation notes.",
        )
        .unwrap();
        let docs_dir = workspace.path().join("docs");
        fs::create_dir_all(&docs_dir).unwrap();
        fs::write(docs_dir.join("wire memo.md"), "Wire memo body.").unwrap();
        fs::write(docs_dir.join(".secret.md"), "Do not copy").unwrap();
        fs::write(workspace.path().join(".env"), "OPENPLANTER_SECRET=1").unwrap();
        fs::write(
            session_dir.join("events.jsonl"),
            serde_json::json!({
                "event_type": "turn.completed",
                "payload": {"text": "# Final Result\n\nSupported finding body."}
            })
            .to_string()
                + "\n",
        )
        ...
        state.evidence.insert(
            ...
            serde_json::json!({
                "id": "trace 1",
                "content": "[trace] retrieval progress only",
                "source_uri": "state.json#external_observations[0]"
            }),
        );
        ...
        assert!(home.contains("[Session Result](Session%20Result.md)"));
        assert!(home.contains("[Content Index](Content%20Index.md)"));
        assert!(home.contains("[Copied Cestus Wiki](../../Wiki/index.md)"));
        assert!(home.contains("[Copied Workspace Artifacts](../../Workspace-Artifacts/Index.md)"));
        let session_result =
            fs::read_to_string(Path::new(&result.investigation_dir).join("Session Result.md"))
                .unwrap();
        assert!(session_result.contains("Supported finding body."));
        assert!(Path::new(&result.root_path).join("Wiki/index.md").exists());
        assert!(
            Path::new(&result.root_path)
                .join("Workspace-Artifacts/docs/wire memo.md")
                .exists()
        );
        assert!(
            !Path::new(&result.root_path)
                .join("Workspace-Artifacts/.env")
                .exists()
        );
        assert!(
            !Path::new(&result.root_path)
                .join("Workspace-Artifacts/docs/.secret.md")
                .exists()
        );
        let content_index =
            fs::read_to_string(Path::new(&result.investigation_dir).join("Content Index.md"))
                .unwrap();
        assert!(content_index.contains("docs/wire memo.md"));
        let findings =
        ...
        let evidence =
            fs::read_to_string(Path::new(&result.investigation_dir).join("Evidence.md")).unwrap();
        assert!(
            evidence.contains("[Source reference](../../Workspace-Artifacts/docs/wire%20memo.md)")
        );
        assert!(!evidence.contains("[trace] retrieval progress only"));
        assert!(!evidence.contains("state.json#"));
        ...
        assert!(manifest_files.iter().any(|path| {
            path.as_str()
                .is_some_and(|path| path == "Workspace-Artifacts/docs/wire memo.md")
        }));
        assert!(manifest_files.iter().any(|path| {
            path.as_str()
