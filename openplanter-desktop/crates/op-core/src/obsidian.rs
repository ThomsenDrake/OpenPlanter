...
fn render_home(
    ...
    if let Some(wiki_index) = &copied_content.wiki_index {
        lines.push(format!(
            "- {}",
            markdown_file_link("Copied OpenPlanter Wiki", &format!("../../{wiki_index}"))
        ));
    }
...
fn render_session_result(
    ...
    let mut lines = vec![
        ...
        String::new(),
        "The final OpenPlanter response is embedded here so the generated pack remains useful inside Obsidian without hidden runtime files.".to_string(),
        String::new(),
    ];
...
fn render_sessions(
    ...
    let mut lines = vec![
        ...
        format!("Back to {}", markdown_link("Home", "Home.md")),
        String::new(),
        "OpenPlanter runtime logs are not copied into the Obsidian vault. Use the session result and generated Markdown notes for the readable investigation surface.".to_string(),
        String::new(),
        format!("- {}", markdown_link("Session Result", "Session Result.md")),
        String::new(),
...
fn render_index(
    ...
    if copied_content.wiki_index.is_some() || copied_content.artifact_index.is_some() {
        ...
        if let Some(wiki_index) = &copied_content.wiki_index {
            lines.push(format!(
                "- {}",
                markdown_file_link("Copied OpenPlanter Wiki", wiki_index)
            ));
        }
        if let Some(artifact_index) = &copied_content.artifact_index {
            lines.push(format!(
...
fn render_artifact_index(artifact_files: &[String], generated_at: &str) -> String {
    ...
    let mut lines = vec![
        frontmatter(
            ...
            "workspace",
            generated_at,
            "OpenPlanter Workspace Artifacts",
        ),
        "# OpenPlanter Workspace Artifacts".to_string(),
        String::new(),
        "Text artifacts copied from the OpenPlanter workspace. Hidden runtime files, credentials, replay logs, node modules, and oversized raw files are excluded.".to_string(),
        String::new(),
    ];
...
mod tests {
    ...
    fn export_pack_writes_markdown_manifest_and_canvas() {
        ...
        assert!(home.contains("[Copied OpenPlanter Wiki](../../Wiki/index.md)"));
